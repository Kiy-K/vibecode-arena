<script lang="ts">
  ...
  const currentPlayerWon = $derived(
    currentPlayerId && sortedPlayers[0]?.id === currentPlayerId
  );
  ...
  function getRankStyle(rank: number): { border: string; bg: string; accent: string; glow: string } {
    if (rank === 1) return {
...
<div class="min-h-screen bg-black text-white flex flex-col">
  ...
  <div class="text-center pt-8 pb-4">
    ...
    <p class="text-neutral-500">Final Results</p>
  </div>
  ...
  <div class="flex-1 flex flex-col justify-center px-4 pb-8">
    <div class="max-w-5xl mx-auto w-full">
      ...
      {#if podiumPlayers.length > 0}
        <div class="flex items-end justify-center gap-4 md:gap-6 mb-8">
          <!-- 2nd Place -->
          {#if podiumPlayers[1]}
            ...
            <div class="flex-1 max-w-[280px] order-1">
              <div class="border-2 {style.border} {style.bg} {style.glow} overflow-hidden">
                <div class="p-4 text-center">
                  ...
                  <div class="text-lg font-semibold truncate mb-3">{displayName(podiumPlayers[1])}</div>
                  ...
                  <div class="space-y-1 text-sm text-neutral-400">
                    <div class="flex justify-between">
                      <span>Model</span>
                      <span class="text-neutral-300 truncate ml-2">{getModelName(podiumPlayers[1].model)}</span>
                    </div>
                    <div class="flex justify-between">
                      <span>Prompts</span>
            ...
            <div class="flex-1 max-w-[320px] order-2 -mb-4">
              <div class="border-2 {style.border} {style.bg} {style.glow} overflow-hidden">
                <div class="p-6 text-center">
                  ...
                  <div class="text-xl font-semibold truncate mb-4">{displayName(podiumPlayers[0])}</div>
                  ...
                  <div class="space-y-1 text-sm text-neutral-400">
                    <div class="flex justify-between">
                      <span>Model</span>
                      <span class="text-neutral-300 truncate ml-2">{getModelName(podiumPlayers[0].model)}</span>
                    </div>
                    <div class="flex justify-between">
                      <span>Prompts</span>
            ...
            <div class="flex-1 max-w-[280px] order-3">
              <div class="border-2 {style.border} {style.bg} {style.glow} overflow-hidden">
                <div class="p-4 text-center">
                  ...
                  <div class="text-lg font-semibold truncate mb-3">{displayName(podiumPlayers[2])}</div>
                  ...
                  <div class="space-y-1 text-sm text-neutral-400">
                    <div class="flex justify-between">
                      <span>Model</span>
                      <span class="text-neutral-300 truncate ml-2">{getModelName(podiumPlayers[2].model)}</span>
                    </div>
                    <div class="flex justify-between">
                      <span>Prompts</span>
      ...
      {#if otherPlayers.length > 0}
        <div class="border-t border-neutral-800 pt-6">
          <h3 class="text-sm text-neutral-500 mb-4 uppercase tracking-wider text-center">Other Players</h3>
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-w-3xl mx-auto">
            {#each otherPlayers as player, i (player.id)}
              <div class="border border-neutral-800 bg-neutral-950/50 p-3 flex items-center justify-between">
                <div class="flex items-center gap-3 min-w-0">
                  <span class="text-neutral-500 font-mono">#{i + 4}</span>
                  <div class="min-w-0">
                    <div class="truncate text-sm">{displayName(player)}</div>
                    <div class="text-xs text-neutral-600 truncate">{getModelName(player.model)}</div>
                  </div>
                </div>
