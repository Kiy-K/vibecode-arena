...
<script lang="ts">
	...
	const timePercent = $derived(challenge ? (timeLeft / challenge.timeLimit) * 100 : 100);
...
>
	...
	<div class="flex items-center justify-between px-3 md:px-6 py-2 md:py-3 gap-2">
		...
		<div class="flex items-center gap-2 md:gap-3">
			...
			<!-- Finished players -->
			<div
				class="flex items-center gap-1 md:gap-2 px-1.5 md:px-2.5 py-1 {submissions.length > 0
			...
			>
				...
				></div>
				<span
					class="text-xs font-mono {submissions.length > 0 ? 'text-green-400' : 'text-neutral-600'}"
				>
		...
		<div class="hidden md:flex absolute left-1/2 -translate-x-1/2 bottom-0 items-end">
			<div class="relative">
				{#if isFrozen}
