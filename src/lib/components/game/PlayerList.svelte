...
<div class="space-y-1">
	<div class="text-xs text-neutral-500 uppercase tracking-wider mb-2 px-2">Players</div>
	{#each sortedPlayers as player (player.id)}
		{@const status = getPlayerStatus(player)}
		{@const config = statusConfig[status]}
		{@const isCurrentPlayer = player.id === currentPlayerId}
		<div
			class="flex items-center gap-2 px-2 py-1.5 rounded {isCurrentPlayer
				? 'bg-neutral-800/50'
				: ''}"
		>
			<!-- Status dot -->
			...
			<div class="flex-1 min-w-0">
				<div class="flex items-center gap-1">
					<span
						class="text-sm truncate {isCurrentPlayer
