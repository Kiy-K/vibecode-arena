...
>
	...
	<div class="flex-1 min-h-[150px] flex flex-col border-b border-neutral-800">
		<div class="px-3 py-1.5 border-b border-neutral-800 bg-black">
			<span class="text-xs uppercase tracking-wider text-neutral-500">Preview</span>
		</div>
		{#if sandboxUrl}
			<div class="flex-1 bg-neutral-900">
				<iframe src={sandboxUrl} title="Component Preview" class="w-full h-full border-0"></iframe>
			</div>
		{:else}
			<div
		...
		>
			<div class="flex items-center justify-between mb-3">
				<div class="flex items-center gap-2">
					<div class="w-2 h-2 rounded-full {result.passed ? 'bg-green-500' : 'bg-red-500'}"></div>
					<span class="text-sm font-bold {result.passed ? 'text-green-400' : 'text-red-400'}">
	...
	<div class="flex-shrink-0 p-4 border-t border-neutral-800 bg-black">
		<button
			...
			class="w-full {isMobile
				...
				? (result?.passed ? 'bg-green-600 text-white' : 'bg-neutral-700 text-neutral-300') +
					' cursor-not-allowed'
				: !hasCodeSource
		...
		{#if !submitted && hasCodeSource && !isMobile}
			<p class="text-center text-xs text-neutral-600 mt-2 hidden md:block">⌘↵ to submit</p>
		{/if}
	</div>
</div>
